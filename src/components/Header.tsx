    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-offwhite/95 backdrop-blur-sm border-b-2 border-bordeaux">
      <div className="container mx-auto px-6 lg:px-12">
        <div className="flex items-end justify-between h-20">
          {/* Logo */}
          <a href="#" className="flex items-end gap-4 pb-0 -mb-[2px]">
            <img 
              src={logo} 
              alt="Van Zoolingen logo" 
              className="h-24 w-auto object-contain"
            />
            <span className="font-serif text-xl md:text-2xl font-semibold text-anthracite tracking-wide pb-4">
              Van Zoolingen
            </span>
          </a>

          {/* Desktop Navigation */}
          <nav className="hidden lg:flex items-center gap-10 pb-4">
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
          ...
          <a
            href="#nieuwsbrief"
            className="hidden lg:block px-6 py-3 mb-4 bg-bordeaux text-primary-foreground text-sm font-medium tracking-wide uppercase hover:bg-bordeaux-dark transition-colors"
          >
            Nieuwsbrief
          </a>
          ...
          <button
            ...
            className="lg:hidden p-2 text-anthracite mb-4"
